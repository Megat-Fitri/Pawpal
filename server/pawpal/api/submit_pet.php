...
$imagesArray = json_decode($_POST['image_paths'], true);
...
try {
    if ($conn->query($sqlinsertpets) === TRUE) {
        ...
        foreach ($imagesArray as $index => $encodedimage) {
            ...
            $imagepath = "../pawpal/pet/pets_" . $last_id . "_" . ($index + 1) . ".png";
            file_put_contents($imagepath, $decodedImage);
            $savedImagePaths[] = "pawpal/pet/pets_" . $last_id . "_" . ($index + 1) . ".png";
        }

        // Save all image paths in DB as comma-separated
        $imagesPathString = implode(",", $savedImagePaths);

        $sqlupdate = "UPDATE tbl_pets SET images_paths = '$imagesPathString' WHERE pet_id = '$last_id'";
